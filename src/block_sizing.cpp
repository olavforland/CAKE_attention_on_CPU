...
int get_num_physical_cores() {
    ...
    if (sysctlbyname("hw.physicalcpu", &cores, &len, NULL, 0) != 0) {
        perror("sysctl hw.physicalcpu failed");
        exit(1);
    }
    return cores;
}
int get_cache_size(int level) {
    ...
    switch (level) {
        case 1:
            // L1 data + instruction is split — typically use L1d
            snprintf(key, sizeof(key), "hw.l1dcachesize");
            break;
        case 2:
			snprintf(key, sizeof(key), "hw.l2cachesize");
            break;
        case 3:
			return 8 * (1 << 20); // 8 MB
        default:
            fprintf(stderr, "Unsupported cache level: %d\n", level);
            exit(1);
    }

    if (sysctlbyname(key, &size, &len, NULL, 0) != 0) {
		perror("sysctl cache size failed");
...
// 	snprintf(command1, sizeof(command1), "grep -c ^processor /proc/cpuinfo");
// 	fp = popen(command1, "r");
...
// 	snprintf(command2, sizeof(command2), "lscpu | grep Thread -m 1 | tr -dc '0-9'");
// 	fp = popen(command2, "r");
...
// 	snprintf(command3, sizeof(command3), "lscpu | grep Socket -m 1 | tr -dc '0-9'");
// 	fp = popen(command3, "r");
...
// 	snprintf(command, sizeof(command), "lscpu | grep Model \
// 					| head -1 \
// 					| tr -dc '0-9'");
// 	fp = popen(command, "r");
...
// 	if(level < 3) {
// 		snprintf(command, sizeof(command), "lscpu --caches=NAME,ONE-SIZE \
// 						| grep L%d \
// 						| grep -Eo '[0-9]*M|[0-9]*K|0-9*G' \
// 						| tr -d '\n'", level);
// 		fp = popen(command, "r");
// 	} else {
// 		snprintf(command, sizeof(command), "lscpu --caches=NAME,ALL-SIZE \
// 						| grep L%d \
// 						| grep -Eo '[0-9]*M|[0-9]*K|0-9*G' \
// 						| tr -d '\n'", level);
// 		fp = popen(command, "r");
